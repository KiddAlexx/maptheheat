...
We're using React 18 and following common best practices focused on clarity, correctness,
and maintainability.
...
This project uses **TanStack React Query + a service layer** for all server state.

- All API reads and writes go through `src/services/` — never call Supabase directly in a component
- React Query hooks (in `src/features/*/hooks/`) wrap service calls and are the only way components access server data
- Client-only UI state (filters, modals, sort order) lives in React Context + useReducer — see `src/context/`
- **Never** put server data in Context; **never** fetch in Context reducers
